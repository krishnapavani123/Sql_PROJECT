-- Active: 1745052354278@@127.0.0.1@3306@zomato
...
--2)Search for any duplicate tripid?
SELECT tripid,count(tripid) from trips_details1
group by tripid having count(tripid)>1;
--No Duplictes are found.
--3)Total no of  Drivers?
select count(DISTINCT driverid) as total_drivers from trips;
--4)Total no of Earnings?
select sum(fare) as fare_amount from trips;
--5)Total no of completed trips?
select count(DISTINCT tripid) from trips;
--6)Total no of Serches?
select sum(searches) as total_seraches from trips_details1;
--7)Searches got Estimate?
select sum(searches_got_estimate ) as searches from trips_details1;
--8)Total trips cancelled by the driver?
select count(*)-sum(driver_not_cancelled) as trips_cancelled_by_drivers from trips_details1;
--9)average distance for trip?
SELECT avg(distance)
FROM trips;
--10)average fare for trip?
select avg(fare) from trips;
--11)Which is the most used payment method?

select a.method from payment a inner join (select  faremethod,count(DISTINCT tripid) from trips
group by faremethod order by count(DISTINCT tripid) desc limit 1)b on a.id=b.faremethod;
--12)which two locations have more number of trips?
select * from (SELECT * ,DENSE_RANK() over(ORDER BY trip desc) as rnk from (select loc_from,loc_to,count(DISTINCT tripid) as trip from trips
group by loc_from,loc_to order by trip desc)a)b where rnk=1;

--UseCase:
--i.Analyzing routes with the highest activity can help assess performance. It can also guide decisions like creating new routes near the popular ones or extending services.
--ii.we can use this insight to optimize services for that route (e.g., adding more vehicles or improving infrastructure).
--iii.Efficiently allocate resources such as drivers, vehicles, or maintenance teams to the most used routes, ensuring customer satisfaction and minimizing downtime.
--13)Top 5 earning drivers?
select * from (select *, DENSE_RANK() over(ORDER BY fare desc) as rnk from 
(select driverid,sum(fare) as fare from trips group by driverid)a)b where rnk<6 ;
--Use Case:
--i.Assign these drivers to premium services or high-demand routes.
--ii.Use insights to train other drivers or refine operational strategies
--iii.Ensure that high performers are paired with reliable vehicles for maximum efficiency

--14)Which duration had more number of trips?
select * FROM
(select * ,rank() over (ORDER BY cnt desc) as rnk FROM
(select duration,count(DISTINCT tripid) as cnt from trips group by duration)a)b where rnk=1;
--Use Case:
--i.Understand which trip durations are most common, indicating peak operational times. This can help with scheduling and planning resources effectively
--ii.Use data insights to predict demand and allocate resources (e.g., vehicles, staff) based on peak trip durations
--iii.Analyze trends to understand customer preferences for trip durations. This information can be leveraged to design services that align with popular durations.
--iii.Adjust pricing models, incentives, or marketing efforts to match durations with the highest trip counts, ensuring alignment with customer behavior.
--15)Which driver and customer pair have more number of trips?
select * from (select *,rank() over (ORDER BY cnt desc)  as rnk from (select driverid,custid,count(distinct tripid) as cnt from trips
group by driverid,custid)a)b where rnk=1;
--Use Case:
--i.can use this data to design driver-specific customer retention strategies.
--ii. By knowing the top customers for each driver, targeted offers or loyalty rewards can be designed to encourage more frequent trips.
--iii.By identifying top customers per driver, logistics teams can streamline assignments (e.g., prioritizing drivers with loyal customers during high-demand periods).

--16)Search to estimate rate

select sum(searches_got_estimate)*100.0/sum(searches) from trips_details1;
--Use Case:
--i.Analyzes how often users receive estimates, which is critical for platforms like ride-hailing services.
--ii.Determines the success rate of returning estimates for user searches, which can measure system reliability or API performance.
--17)Which area got highest number of trips?
select DISTINCT c.assembly1 ,d.* from loc c inner join (select * from (select * , rank() over (ORDER BY cnt desc) as rnk from 
(select loc_from ,count(DISTINCT tripid) as cnt from trips group by loc_from)a)b where rnk=1 )d on c.id=d.loc_from ;
--Use Case:
--i.Highlights areas with significant activity, indicating potential hotspots for transportation demand.
--ii. Provides a high-level view of demand density across assembly areas, which can guide regional planning.

--18)which area got highest no of trips in which duration?
select * from (select * ,rank() over (PARTITION BY duration ORDER BY cnt desc) as rnk  from (select duration,loc_from,count(DISTINCT tripid) as cnt from trips
group by  duration,loc_from)a)b where rnk=1;
--Use Case:
--i.Ensures that vehicles or services are concentrated where they’re most needed.
--ii. insights into how trip demand varies across different durations and locations.
--19)Which location got highest no of fares?
select DISTINCT a.assembly1 from loc a inner join (select * from (select *,rank() over (ORDER BY fare desc) as rnk from
 (select loc_from,sum(fare) as fare from trips group by loc_from)a)b where rnk=1)c on a.id=c.loc_from ;
--Use Case:
--i.Aggregates revenue insights at the assembly area level, making it easier to identify regions contributing most to revenue.
--ii. Increasing driver availability or enhancing customer experience in these areas.
--iii.Focuses on the peak revenue contributors without diluting the results with secondary or lower-ranked locations
--20)which location got more cancellations(by driver)?
select DISTINCT c.assembly1 from loc c  inner join (select * from (select * ,rank() over (order by cancelld_trips desc) as rnk from (select loc_from,count(*)-sum(driver_not_cancelled) as cancelld_trips from trips_details1
group by loc_from)a)b where rnk=1)d on c.id=d.loc_from;
--Use Case:
--i.Adjust driver schedules based on high cancellation trends during specific times or locations
--ii. Encourages repeat trips and compensates for inconvenience caused by cancellations.
--iii.imporve or make some shorcut ways from those areas and allocate more no of drivers of that particular areas.
--21)which location got more cancellations(by customer)?
select DISTINCT c.assembly1 from loc c  inner join (select * from (select * ,rank() over (order by cancelld_trips desc) as rnk from (select loc_from,count(*)-sum(customer_not_cancelled) as cancelld_trips from trips_details1
group by loc_from)a)b where rnk=1)d on c.id=d.loc_from;
select * from (select * ,rank() over (order by cancelld_trips desc) as rnk from (select loc_from,count(*)-sum(customer_not_cancelled) as cancelld_trips from trips_details1 
group by loc_from having cancelld_trips> 0)a)b where rnk=1
...
GROUP BY tripid
HAVING COUNT(*) > 1;
--23)Detect inconsistent data types or range errors
SELECT tripid, distance
FROM trips
WHERE distance < 0 OR duration < 0;
--24)Identify NULL or missing values in critical columns
SELECT tripid, faremethod, fare, driverid, custid
FROM trips
WHERE fare IS NULL OR driverid IS NULL OR custid IS NULL;


--25)which routes generate the most revenue?
SELECT loc_from, loc_to, SUM(fare) AS total_revenue
FROM trips
GROUP BY loc_from, loc_to
ORDER BY total_revenue DESC;
--Use Case:
--i.Focus marketing and operational resources on lucrative routes.


--26)Who are frequent customers and their activity levels?
SELECT custid, COUNT(DISTINCT tripid) AS trip_count, SUM(fare) AS total_spent
FROM trips
GROUP BY custid
ORDER BY total_spent DESC;
--Use Case:
--i.Target top customers with loyalty rewards or personalized offers

--27)Analyze driver or customer cancellation trends by location.
SELECT loc_from, 
       COUNT(*) AS total_trips, 
       (COUNT(*) - SUM(driver_not_cancelled)) AS driver_cancellations, 
       (COUNT(*) - SUM(customer_not_cancelled)) AS customer_cancellations
FROM trips_details1
GROUP BY loc_from
ORDER BY driver_cancellations DESC, customer_cancellations DESC;
--Use Case:
--i.Pinpoint areas with high cancellations and address root causes.
--28)Track trips and earnings on a monthly basis for a trend analysis.

--Conclusion:
--This project demonstrates expertise in data analysis, SQL, and Power BI integration
--by providing actionable insights into transportation operations. 
--It highlights critical metrics like trip efficiency, revenue trends, and cancellation patterns,
--enabling strategic decision-making. The use of advanced queries and visualizations 
--ensures scalability, efficiency, and customer satisfaction, showcasing a strong foundation 
